...
Notes:
\begin{itemize}
\item Object names are case-sensitive
\item Object names should be meaningful and short (``best practice'')
\item \texttt{<-} is most common, but \texttt{=} is acceptable
\end{itemize}
...
Objects can be numbers, strings, matrices, or even more complicated \texttt{R} objects.\
Examples of \texttt{R} object types:
\begin{itemize}
\item the computing basics: integer, numeric, vector, matrix, list
\item data.frame
\item factor
\item lm object (linear model object)
\item formula
\end{itemize}
...
\begin{itemize}
\item `echo`: logical argument determining whether the code in the subsequent chunk will be printed in the document created
\item `eval`: logical argument determining whether or not the code in the subsequent chunk will be evaluated by `{R}$ or just printed in the document.
\item `fig.cap`: provides a caption for a figure
\item `fig.align`: aligns figure on "left", "right" or "center"
\end{itemize}
